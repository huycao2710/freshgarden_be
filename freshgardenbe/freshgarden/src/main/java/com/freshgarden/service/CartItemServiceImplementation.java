...
import com.freshgarden.repository.CartItemRepository;
...
public class CartItemServiceImplementation implements CartItemService{
	...
	public CartItem createCartItem(CartItem cartItem) {
		cartItem.setCountInStock(1);
		cartItem.setPrice(cartItem.getProduct().getPrice()*cartItem.getCountInStock());
		cartItem.setDiscountedPrice(cartItem.getProduct().getDiscountedPrice()*cartItem.getCountInStock());
	...
	public CartItem updateCartItem(Long userId, Long id, CartItem cartItem) throws CartItemException, UserException {
		...
		if(user.getId().equals(userId)) {
			item.setCountInStock(cartItem.getCountInStock());
			item.setPrice(item.getCountInStock()*item.getProduct().getPrice());
			item.setDiscountedPrice(item.getProduct().getDiscountedPrice()*item.getCountInStock());
		}
